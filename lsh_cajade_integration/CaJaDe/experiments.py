...
from src.lsh_graph_generator import LSH_Graph_Generator
...
def main():
  ...
  parser.add_argument('-H','--db_host', metavar="\b", type=str, default='127.0.0.1',
  ...
  user_query = "select location, zip_code, sum(ytd) as ytd_sum from libraries_visitors_by_location group by zip_code, location;"
  u_query = (user_query, 'demo')
  u_question =["location='Chicago Lawn'","location='West Lawn'"]
  user_specified_attrs = [('location','location')]  
  ...
  if(args.workloads=='false' and args.case_studies=='false'):
    run_experiment(
      ...
      user_questions_map = {'yes':'Chicago Lawn', 'no':'West Lawn'},
      user_specified_attrs=user_specified_attrs,
