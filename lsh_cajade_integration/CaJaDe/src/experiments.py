...
def run_experiment(conn=None,
    ...
    query_from_table = "public_health_statistics__WWWworkplace@098diabetes_hospitalizations_in_chicago_"
...
def main():
  ...
  user_query = "provenance of (select location, year, sum (CAST(ytd AS int)) from libraries_visitors_by_location where location = 'Chicago Lawn' group by location, year);"
  # u_query = (user_query, 'gsw wins : 15 vs 12') 
  u_query = (user_query, 'demo')
  u_question =["year='2019'","year='2020'"]
  user_specified_attrs = [('location','location'),('sum','total_visits')]
  ...
  if(args.workloads=='false' and args.case_studies=='false'):
    run_experiment(
      ...
      user_questions_map = {'yes':'2019', 'no':'2020'},
      user_specified_attrs=user_specified_attrs,
